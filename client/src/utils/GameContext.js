...
export default function GameProvider({ children }) {
      ...
      if (isCorrect) {
        try {
          const { data, error } = await updateGame({
            variables: {
              gameId,
              gameData: JSON.stringify({ gameArray: sudokuArray }, (key, val) =>
                key === "candidates" ? [...val] : val
              ),
              elapsedTime,
              isSolved: true,
            },
          ...
          if (data?.updateGame.stats.length) {
            setMessage(
              "You won!\n" +
                JSON.stringify(data?.updateGame.stats, (key, val) =>
                  key[0] === "_" ? undefined : val
                )
            );
            setOverlay({
              show: true,
  ...
  async function saveNewGame(sudokuArray, difficulty) {
    try {
      ...
      if (data.addGame._id) {
        ...
        setMessage(
          "Something went wrong saving new game. (Open dev console for help.)"
        );
  ...
  async function toggleCandidate(candidate, cellRef) {
    const updatedArray = toggleCandidateHandler(
      gameArray,
      selected,
      modeMultiselect,
      candidate,
  ...
  async function fillCandidates(options) {
    ...
    if (options?.all) {
      for (let row = 0; row < 9; row++) {
        for (let col = 0; col < 9; col++) {
          updatedArray[row][col].candidates = new Set([
            1, 2, 3, 4, 5, 6, 7, 8, 9,
          ]);
        }
      }
  ...
  async function loadDifficulty(difficulty) {
    ...
    getBoardByDifficulty(difficulty).then(async (board) => {
      ...
      if (
        board?.newboard?.grids?.[0]?.value &&
        board?.newboard?.grids?.[0]?.solution
      ) {
        for (let row = 0; row < 9; row++) {
          for (let col = 0; col < 9; col++) {
